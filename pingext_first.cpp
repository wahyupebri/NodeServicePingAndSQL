...
void DecodeICMPHeader(char* buf, int bytes,
	struct sockaddr_in* from, int *time)
{
	IpHeader* iphdr = NULL;
	...
	if(bytes)
	*time = tick - icmphdr->timestamp;//add by wahyu
...
// 
int doPing(char* ip, int *timeduration) {
	WSADATA wsaData;
	...
	while (1)
	{
		...
		//*timeduration = -1;
		DecodeICMPHeader(recvbuf, bread, &from, timeduration);
		printf("%s : %d\n", ip, *timeduration);
		if (*timeduration >= 0)
			break;
		Sleep(1000);
